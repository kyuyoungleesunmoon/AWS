# AWS 입문자를 위한 완벽 실습 가이드 (2026년 최신판)

이 문서는 AWS 클라우드를 처음 접하는 엔지니어를 위해 작성되었습니다. 계정 보안부터 서버 구축, 데이터베이스 연결까지, **실무에서 사용하는 방식**과 **학습용 편의 설정**을 구분하여 단계별로 설명합니다.

> **⚠️ 작성 기준일**: 2026년 1월 2일
> **환경**: Windows 10/11, Chrome 브라우저
> **필수 준비물**: AWS 루트 계정, [MobaXterm Free](https://mobaxterm.mobatek.net/download.html) 설치
...
5.  **키 페어(로그인)** ⭐ 중요:
    ...
    *   **[키 페어 생성]** 클릭
    *   ⚠️ **자동으로 다운로드되는 `my-key-2026.pem` 파일을 안전한 곳에 보관** (예: `C:\AWS\`)
    *   **분실 시 복구 불가능!**
...
7.  **연결 (핵심 설정 ⭐⭐⭐)**: ← **이 단계가 외부 접속 성공의 90%를 결정합니다!**
    ...
    *   **현실 상황**: 대부분의 경우 `rds-ec2-db-subnet-group-1` (4 서브넷, 4 가용 영역)만 보입니다.
        ...
        *   ⚠️ **중요**: 생성 후 5-2 섹션 "라우팅 테이블 수정" 필수!
...
11. **서브넷 확인**:
    ...
    *   **서브넷 그룹** 항목 확인:
        *   ✅ `default` → 바로 12단계로
        *   ⚠️ `rds-ec2-db-subnet-group-x` → 5-2 섹션 "라우팅 설정" 필수!
...
5.  **[규칙 추가]** 버튼을 눌러 다음 규칙을 추가:
    ...
    *   설명: `EC2 access`
    *   ⚠️ **주의**: 소스에 자기 자신(`rds-public-2026`) 넣으면 안 됩니다!
...
2.  **라우팅 테이블 ID 링크** 클릭 (예: `rtb-0975463209b1a1019 | RDS-Pvt-rt`)
    *   ⚠️ **주의**: "라우팅 테이블 연결 편집" 버튼 누르면 안 됩니다! **링크를 클릭**하세요.
...
#### **주의사항**
- ⚠️ **터널은 MobaXterm이 실행 중일 때만 작동**합니다
- ⚠️ MobaXterm 종료 → 터널 중지 → Workbench 접속 불가
- ⚠️ 터널 사용 시 RDS 엔드포인트로 직접 접속 불가 (127.0.0.1만 사용)
- ⚠️ PC 재부팅 시 터널 자동 시작 안 됨 (수동으로 [Start] 클릭)
